...
from sterodistance.depth_SGBM import depth_SGBM
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        if x < 640:
                            ...
                            dy = int((y2 - y1) / 4)
                            sum = 0
                            for i in range(x1+dx, x2, dx):
                                for j in range(y1+dy, y2, dy):
                                    if i < 640 and i <= x2 and j <= y2:
                                        print("i:%d, j:%d",i,j)
                                        print("x2:%d, y2:%d",x2,y2)
                                        dis_0 = abs(threeD[j][i][2]) 
                                        if dis_0 >0 and dis_0 < 3000:
                                            sum += dis_0
                            sum /= 9
                            label = None if hide_labels else (names[c] if hide_conf else f'{names[c]} {conf:.2f} {sum}')
