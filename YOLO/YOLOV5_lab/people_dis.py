...
# 导入图片警示标注
logo_warning = cv2.imread("./logo/people1.png")
h_logo_warning, w_logo_warning, _ = logo_warning.shape
logo_stop = cv2.imread("./logo/people2.png")
h_logo_stop, w_logo_stop, _ = logo_stop.shape
logo_flag = cv2.imread("./logo/doc.jpg")
h_logo_flag, w_logo_flag, _ = logo_flag.shape
red_dis = cv2.imread("./logo/red_dis.png")
yellow_dis = cv2.imread("./logo/yellow_dis.png")
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for c in det[:, -1].unique():
                    if names[int(c)] == "people":
                        man_flag = True
                    if names[int(c)] == "Chemicals":
                        che_flag = True
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        if x < 640:
                            ...
                            for i in range(x1 + dx, x2):
                                for j in range(y1 + dy, y2):
                                    if i < 640 and i <= x2 and j <= y2:
                                        dis_0 = abs(threeD[j][i][2])
                                        if dis_0 > 0 and dis_0 < 5000:
                                            if flag:
                                                flag = False
                                                point_dis[0] = threeD[j][i][0]
                                                point_dis[1] = threeD[j][i][1]
                                                point_dis[2] = threeD[j][i][2]
                                            count += 1
                                            sum += dis_0
                            if names[c] == "Chemicals":
                                che_pixel = [int(x), int(y)]
                                if flag:
                                    che_point = [0, 0, 0]
                                else:
                                    che_point[0] = point_dis[0]
                                    che_point[1] = point_dis[1]
                                    che_point[2] = point_dis[2]
                            if names[c] == "people":
                                man_pixel = [int(x), int(y)]
                                if flag:
                                    man_point = [0, 0, 0]
                                else:
                                    man_point[0] = point_dis[0]
                                    man_point[1] = point_dis[1]
                                    man_point[2] = point_dis[2]
                            # 危险物间距检测报警模块————人物间距
                            if man_flag and che_flag:
                                cv2.line(im0, man_pixel, che_pixel, (0, 0, 255), 2)
                                # 中心连线
                                if (man_point[2] == 0) or (che_point[2] == 0):
                                    pass
                                else:
                                    # man_che_dis = (abs(abs(man_point[0]) - abs(che_point[0])) ** 2 + abs(
                                    #     abs(man_point[1]) - abs(che_point[1])) ** 2 + abs(
                                    #     abs(man_point[2]) - abs(che_point[2])) ** 2) ** 0.5
                                    dis = (abs(abs(man_pixel[0]) - abs(che_pixel[0])) ** 2 + abs(
                                        abs(man_pixel[1]) - abs(che_pixel[1])) ** 2) ** 0.5 - 35
                                    print(dis)
                                    if (dis < 500):
                                        if dis > 300:
                                            im0[:, :] = merge_img(im0, 0.3, logo_warning, 0.7, 0, (0, 0))
                                            im0[:, :] = merge_img(im0, 0.3, yellow_dis, 0.7, 0, (0, 50))
                                            cv2.putText(im0, f'{dis:.0f}cm', (60, 68), cv2.FONT_HERSHEY_COMPLEX, 0.6,
                                                        (0, 0, 255), 2)


                                        elif dis > 0:
                                            im0[:, :] = merge_img(im0, 0.3, logo_stop, 0.7, 0, (0, 0))
                                            im0[:, :] = merge_img(im0, 0.3, red_dis, 0.7, 0, (0, 50))
                                            cv2.putText(im0, f'{dis:.0f}cm', (60, 68), cv2.FONT_HERSHEY_COMPLEX, 0.6,
                                                        (0, 255, 255), 2)
                                            if dis < 150:
                                                doc = cv2.resize(logo_flag, (200, 300))
                                                im0[:, :] = merge_img(im0, 0.6, doc, 0.4, 0, (440, 115))
                            ...
                            if (names[c] == "warning") and (y < 300) and (x < 450):
                                # logosize = 200
                                # global logo_flag
                                # logo_flag = cv2.resize(logo_flag, (logosize, logosize))
                                # im0_logo = im0[-logosize:, -logosize:]
                                # overlap = cv2.addWeighted(logo_flag, 0.8, im0_logo, 0.2, 0)
                                # im0[-logosize:, -logosize:] = overlap
                                # if (sum > 150) and (sum < 300):
                                #     im0[:h_logo_warning, :w_logo_warning] = logo_warning
                                # elif (sum > 0) and (sum < 150):
                                #     im0[:h_logo_stop, :w_logo_stop] = logo_stop
                                logosize = 200
                                logo_flag = cv2.resize(logo_flag, (logosize, logosize))
                                im0[:, :] = merge_img(im0, 0.2, logo_flag, 0.8, 0, (640 - logosize, 480 - logosize))
                                if (sum > 150) and (sum < 300):
                                    # im0[:h_logo_warning, :w_logo_warning] = logo_warning
                                    im0[:, :] = merge_img(im0, 0.3, logo_warning, 0.7, 0, (0, 0))
                                elif (sum > 0) and (sum < 150):
