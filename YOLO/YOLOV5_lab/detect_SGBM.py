...
from distance.depth_SGBM import depth_SGBM
...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s) # 权重文件地址 默认 weights/可以是自己的路径
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            # 测距部分
            global threeD
            out_line, disp, fakeColorDepth, threeD = depth_SGBM(img_origin, iml, imr, height_0, width_0)
...
#                                         print("x2:%d, y2:%d",x2,y2)
                            ...
                            if count == 0 :
                                label = None if hide_labels else (
                                    names[c] if hide_conf else f'{names[c]} cre:{conf:.2f} -')
                            else :
                                sum = sum / ( count * 10)
                                label = None if hide_labels else (names[c] if hide_conf else f'{names[c]} cre:{conf:.2f}  dis:{sum:.2f}cm')
            ...
            if view_img:
                ...
                if cv2.waitKey(1) & 0xFF == 27:  # ESCq to quit
            ...
            if save_img:
                if dataset.mode == 'image':
                    ...
                    if vid_path != save_path:  # new video
                        ...
                        vid_writer2 = cv2.VideoWriter(str(Path(str(save_dir / 'show.avi')).with_suffix('.mp4')), cv2.VideoWriter_fourcc(*'mp4v'), fps, (iml_show.shape[1], iml_show.shape[0]))
